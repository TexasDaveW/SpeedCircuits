...
var tiles = [
  ...
  { instanceId: "resistor-1", catalogId: "resistor-470", gridX: 5, gridY: 4, rotation: 90 },
  { instanceId: "led-1", catalogId: "led-red", gridX: 5, gridY: 5, rotation: 270 },
  { instanceId: "ground-1", catalogId: "ground-tile", gridX: 5, gridY: 6, rotation: 0 }
];
var outDir = join(process.cwd(), "circuit jsons");
mkdirSync(outDir, { recursive: true });
var doc = exportCircuit(tiles, "LED + Resistor");
writeFileSync(join(outDir, "01-led-plus-resistor.json"), `${JSON.stringify(doc, null, 2)}
`);
var tileConn = buildConnections(tiles).filter((c) => !("net" in c.b));
console.log("tile-tile connections:", tileConn.length);
for (const c of tileConn) console.log(c);
